    ...
    <div className="min-h-screen bg-gradient-to-r from-yellow-300 to-green-400 flex items-center justify-center">
      <form onSubmit={handleForm} className="bg-white p-6 rounded shadow-md w-full max-w-sm flex items-center flex-col">
        ...
        <Input
          type="email"
          name="email"
          className="border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-lime-400 focus:border-transparent transition duration-200 mb-2"
        ...
        <Input
          type="password"
          name="password"
          className='border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-lime-400 focus:border-transparent transition duration-200'
          placeholder="Digite sua senha"
          required={true}
          value={formData.password}
          onChange={handleChange}
        />
        <button type="submit" className="mt-2 bg-yellow-400 text-black px-5 py-2 rounded-md">Entrar</button>
        ...
        <p>Esqueceu sua senha? <a className="text-lime-700 cursor-pointer" href=''>Redefinir senha</a></p>
        <p className="mt-4">Primeira vez aqui? <Link href="/register" className="text-lime-700">Crie uma conta!</Link></p>
      </form>
    </div>
  );
}
